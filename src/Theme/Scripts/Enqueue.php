...
class Enqueue {
	...
	public function enqueue( bool $in_footer = true ): void {
		$this->handle->get_manifest()->enqueue( $in_footer );
	}
	...
	public function register(): void {
		if ( Util::in()->is_javascript_resource( $this->handle ) ) {
			wp_register_script( $this->handle->handle(), $this->handle->get_manifest()->get_url(), $this->handle->dependencies(), $this->get_version(), true );
		} else {
			wp_register_style( $this->handle->handle(), $this->handle->get_manifest()->get_url(), $this->handle->dependencies(), $this->get_version() );
		}
	}
	...
	public function get_file( bool $full_path = false ): string {
		return $this->handle->get_manifest()->get_file( $full_path );
	}
	...
	public function get_version(): string {
		return $this->handle->get_manifest()->get_version();
	}
	...
	public function get_integrity(): string {
		return $this->handle->get_manifest()->get_integrity();
	}
